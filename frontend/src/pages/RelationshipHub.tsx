...
export default function RelationshipHub() {
  ...
  const renderNoPartnerPlaceholder = (title: string, description: string) => {
    return (
      <motion.div 
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-md mx-auto glass-card p-8 text-center space-y-6 border border-rose-500/10 bg-rose-500/5 mt-10"
      >
        <div className="w-16 h-16 rounded-full bg-rose-500/10 border border-rose-500/20 flex items-center justify-center mx-auto text-rose-500">
          <Heart className="w-8 h-8 animate-pulse text-rose-500 fill-rose-500/20" />
        </div>
        
        <div>
          <h4 className="text-lg font-black text-white">{title}</h4>
          <p className="text-xs text-[var(--text-secondary)] mt-2">{description}</p>
        </div>
        ...
        {friendsList.length > 0 ? (
          <div className="space-y-3 pt-2">
            ...
            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-1">
              {friendsList.map((f: any) => (
                <button
                  key={f.id}
                  onClick={() => setSelectedPartner(f.id)}
                  type="button"
                  className="w-full flex items-center gap-3 p-2.5 rounded-xl border border-white/5 bg-white/3 hover:bg-rose-500/10 hover:border-rose-500/20 text-left transition-all cursor-pointer text-xs font-semibold text-white"
                >
                  <img 
                    src={f.profile?.avatar_url || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=200&auto=format&fit=crop'} 
                    alt="" 
                    className="w-8 h-8 rounded-full object-cover border border-white/10"
                  />
                  <div>
                    <span className="block font-bold">{f.profile?.full_name || f.username}</span>
                    <span className="text-[10px] text-[var(--text-secondary)]">@{f.username}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4 pt-2">
            <p className="text-xs text-amber-400 bg-amber-500/10 border border-amber-500/25 p-3 rounded-xl">
              You don't have any connections in your friend list yet. Add friends on the Discovery feed first to unlock all features!
            </p>
            <button
              onClick={() => {
                navigate('/', { state: { tab: 'discovery' } })
              }}
              type="button"
              className="w-full py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:brightness-110 text-white font-bold text-xs transition-all shadow-md shadow-blue-500/20 cursor-pointer"
            >
              Explore Home Feed
            </button>
          </div>
    ...
    <div className="space-y-6 h-full flex flex-col overflow-hidden p-6 bg-transparent">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {activeTab === 'compatibility' && (
          !selectedPartner ? (
            renderNoPartnerPlaceholder("Compatibility Meter", "Compatibility is calculated dynamically by overlaying shared hobbies, music styles, movies, and interests between two profiles.")
          ) : (
        ...
        {activeTab === 'anniversaries' && (
          !selectedPartner ? (
            renderNoPartnerPlaceholder("Anniversary Calendar", "Keep track of significant dates, first proposals, and relationship milestones with dynamic countdowns and notification alerts.")
          ) : (
        ...
        {activeTab === 'memories' && (
          !selectedPartner ? (
            renderNoPartnerPlaceholder("Secure Memory Vault", "Store private photos, letters, voice notes, and keepsakes in an encrypted repository built for two.")
          ) : (
        ...
        {activeTab === 'timeline' && (
          !selectedPartner ? (
            renderNoPartnerPlaceholder("Couple Memory Timeline", "An AI-powered historical recap visualizing your shared moments, chats, and anniversaries over time.")
          ) : (
            <AIMemoryView 
